...
export class AnalysisController extends BaseController {
  ...
  async getTotalData() {
    ...
    // 昨日用户
    const yesterdayUserSql = `SELECT count(*) as total  FROM ${this.userService.getTableName()} where DATE_FORMAT(createTime, '%Y%c%e') = '${dateStr}'`;
    const yesterdayUserRes = await this.userService.userModel.query(
      yesterdayUserSql
    );

    // 昨日订单
    const yesterdayOrderCompleteSql = `SELECT count(*) as total  FROM ${this.orderService.getTableName()} where DATE_FORMAT(createTime, '%Y%c%e') = '${dateStr}' and status = 4`;
    const yesterdayOrderCompleteTotal =
      await this.orderService.orderEntity.query(yesterdayOrderCompleteSql);
    // 昨日交易额
    const yesterdayTradeSumSql = `select sum(payAmount) as value from ${this.orderService.getTableName()} where DATE_FORMAT(createTime, '%Y%c%e') = '${dateStr}' and status=4`;
    const yesterdayTradeSum = await await this.orderService.orderEntity.query(
      yesterdayTradeSumSql
    );

    // 昨日盈利
    const yesterdayIncomeTotalSql = `select sum(platformIncome) as value from ${this.orderService.balanceSheetEntity.metadata.tableName} where DATE_FORMAT(createTime, '%Y%c%e') = '${dateStr}'`;
  ...
  async getOrderStatus(@Query() dto: AnalysisDayDTO) {
    const sql = `select count(*) as total from ${this.orderService.getTableName()} where DATE_FORMAT(createTime, '%Y%m%d') >= '${
      dto.beginDate
    }' and DATE_FORMAT(createTime, '%Y%m%d')<= '${dto.endDate}' and `;
    const [
      cancel,
      close,
      waitPay,
  ...
  async getNewUser(@Query() dto: AnalysisDayDTO) {
    ...
    const userCountDaily = await this.userService.userModel.query(
      `select count(*) as count, date_format(createTime, '%Y-%m-%d') as createDate FROM ${this.userService.getTableName()} ${where}`
    );
    const wxCountDaily = await this.userService.userModel.query(
      `select count(*) as count, date_format(createTime, '%Y-%m-%d') as createDate FROM ${this.wxappService.getTableName()} ${where}`
    );
