...
export class ConfigController extends BaseController {
  ...
  @Post('/corwx')
  @Validate()
  async corwxPost(@Body() dto: ConfigCorwxDTO) {
    await this.configService.upsert(CONFIG_CORWX_INFO, dto);
    return this.responseSuccess('更新配置成功');
  }

  @Get('/corwx')
  async corwxGet() {
    const result = await this.configService.getConfig(CONFIG_CORWX_INFO);
    return this.responseSuccess('ok', result);
  }
}
