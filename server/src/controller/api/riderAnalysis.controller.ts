...
export class RiderAnalysisController extends BaseController {
  ...
  async getData() {
    ...
    const today = await this.riderService.cashEntity.query(
      `select count(*) as total from ${this.orderService.getTableName()} where date_format(successTime,'%Y%c%e') = '${dateValue}' and status=4`
    );
    // 本月成单
    ...
    const all = await this.riderService.cashEntity.query(
      `select count(*) as total from ${this.orderService.getTableName()} where status=4`
    );
