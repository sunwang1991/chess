...
export class UserCouponController extends BaseController {
  ...
  async getUnreadCoupon() {
    const result = await this.queryService.select(
      ...
      {
        tables: `${this.couponService.userCouponEntity.metadata.tableName} uc , ${this.couponService.couponEntity.metadata.tableName} co`,
  ...
  async postReadCoupon(@Body('ids') ids: number[]) {
    await this.couponService.userCouponEntity.query(
      `update ${
        this.couponService.userCouponEntity.metadata.tableName
      } set isLook=1 where id in (${ids.toString()})`
    );
